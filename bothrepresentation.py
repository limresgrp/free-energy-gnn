...
def read_dataset(train_perc, compact_repr):
...
def objective():
    ...
    train_perc = 0.1
    dataset, train_ind, validation_ind, test_ind, target_mean, target_std = read_dataset(train_perc, True)
    dataset2, train_ind, validation_ind, test_ind, target_mean, target_std = read_dataset(train_perc, False)
    for model, hyperparameters in define_model(dataset[0]):
        ...
        for i in range(run_parameters["epochs"]):
            ...
            for number, j in enumerate(tqdm.tqdm(train_ind)):
                ...
                # Forward pass for dataset 2: Compute predicted y by passing x to the model
                y_pred = model(dataset2[j].to(device))
                # Compute and print loss
                loss = criterion(y_pred, dataset2[j].y)
                # Zero gradients, perform a backward pass, and update the weights.
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            with torch.no_grad():
                for j in validation_ind:
                    ...
                    y_pred = model(dataset2[j].to(device))
                    val_loss = criterion(y_pred, dataset2[j].y)
                    val_losses.append(val_loss.item())
